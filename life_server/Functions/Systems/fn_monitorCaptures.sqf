         ...
         if (!(worldName == "Stratis" && !(_x select 4))) then {
             ...
             if (time - (_x select 3) > 900) then {
                 ...
                 _rowUpdated = false; {
                     ...
                     if (_x in playableUnits && ((getPosATL _x) distance _pointPos) < 50) then {
                         ...
                         _gang = group _x getVariable["gang_id", 0];
                         if (_gang > 0 /* && (!(surfaceIsWater (position _x)) || _id == 3) && !(primaryWeapon _x in ["","Binocular","SMG_02_F","hgun_PDW2000_F","arifle_sdar_F"])*/ ) then {
                             if (!(_gang in _totals)) then {
                                 _totals set[count _totals, _gang];
                             };
                         };
                     };
                 }
                 ...
                 if (count _totals == 1) then {
                     ...
                     if ((_totals select 0) == (_x select 1)) then {
                             ...
                             if (_new == 1) then {
                                 ...
                                 /*_row = _x; {
                                     if (_x getVariable["gang", "0"] == _row select 1) then {
                                         [
                                             [2, 5], "life_fnc_addStatistic", _x, false
                                         ] spawn life_fnc_MP;
                                     };
                                 }
                                 forEach _players;*/
                         ...
                         if ((_x select 2) > 0) then {
                             ...
                             /*[
                                [_id], "life_fnc_capNotice", civilian, false
                             ] spawn life_fnc_MP;*/
                         };
                     };
                 ...
                 _row = _x; {
                     [
                         [2, format["This area was recently captured and cannot be again for another %1 + minutes.", floor((900 - (time - (_row select 3))) / 60)]], "life_fnc_broadcast", _x, false
                     ] spawn life_fnc_MP;
                 }
                 forEach _players;
             };
             ...
             switch (true) do {
                 case ((_x select 2) == 1):{
                         ...
                         _gangName = "";
                         {
                             if ((_x getVariable "gang_id") == (_point select 1)) then {
                                 _gangName = _x getVariable "gang_name";
                             };
                         } foreach allGroups;
                         if (_gangName == "") exitWith {};
                         _newText = format["%1 - %2", (_point select 0), _gangName];
                     };
                 default {
                     _newText = format["%1 - Contested", (_x select 0)];
                 };
             };
