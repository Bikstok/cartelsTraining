...
/*
if (count life_server_gangs > 0) then {
    {
        diag_log format["Gang: %1 == %2", _x select 0, _queryResult select 0];
        if !(_x select 0 == _queryResult select 0) then {
            life_server_gangs pushBack _queryResult;
        };
    } forEach life_server_gangs;
} else {
    diag_log format["Gang2: %1", _queryResult select 0];
    life_server_gangs pushBack _queryResult;
};
diag_log format["Gangs: %1", life_server_gangs];*/
