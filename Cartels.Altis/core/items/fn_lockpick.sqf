...
life_action_in_use = true; //Lock out other actions
...
if(!alive player OR life_istazed) exitWith {life_action_in_use = false;};
if((player getVariable["restrained",false])) exitWith {life_action_in_use = false;};
if(!isNil "_badDistance") exitWith {titleText[localize "STR_ISTR_Lock_TooFar","PLAIN"]; life_action_in_use = false;};
if(life_interrupted) exitWith {life_interrupted = false; titleText[localize "STR_NOTF_ActionCancel","PLAIN"]; life_action_in_use = false;};
if(!([false,"lockpick",1] call life_fnc_handleInv)) exitWith {life_action_in_use = false;};
